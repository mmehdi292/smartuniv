...
<body>
		...
		<div class="tableDiv">
			...
			<div class="text">
				...
				<div class="opt">
					...
					</label> </label> <label>Role <select onselect="">
							<option value="">Aucan</option>
	...
	<c:forEach var="ens" items="${sessionScope.enseignents}">
			var item = {nom:"${ens.getNom()}",
					prenom:"${ens.getPrenom()}",
					username:"${ens.getUsername()}",
					email:"${ens.getEmail()}",
					dn:"${ens.getDn()}",
					ln:"${ens.getLn()}",
					sexe:"${ens.getSexe()}"
			};
			document.getElementById("table").innerHTML="<tr><td>{{item.nom}}</td><td>{{item.prenom}}</td><td>{{item.username}}</td><td>{{item.email}}</td><td>{{item.dn}}</td><td>{{item.ln}}</td><td>{{item.sexe}}</td><td><a href="ModifierActeur?type=enseignent&username={{item.username}}"><i class="fas fa-user-edit fa-lg"></i></a> <a href="SupprimerActeur?type=enseignent&username={{item.username}}" onclick="return(confirm('Etes-vous sûr de vouloir supprimer \n {{item.nom}} {{item.prenom}}'));"><i class="fas fa-trash-alt fa-lg"></i></a></td></tr>";
